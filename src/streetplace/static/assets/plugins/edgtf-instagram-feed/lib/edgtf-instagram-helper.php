...
/**
 * Class EdgefInstagramHelper
 */
class EdgefInstagramHelper
{
    /**
     * Generates HTML for given image from Instagram feed. Defines edgtf_instagram_image_atts filter
    ...
    public function getImageHTML($image, $imageSize) {
        ...
        $imageAtts = apply_filters('edgtf_instagram_image_atts',
            array(
                'class' => $imageSize,
    ...
    public function getImageAlt($imageArr) {
        $imageAlt = '';
        if (is_array($imageArr) && count($imageArr)) {
            $imageCaption = ($imageArr['caption']) ? $imageArr['caption'] : array();
