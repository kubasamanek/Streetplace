...
include_once 'edgtf-twitter-helper.php';

/**
 * Class EdgefTwitterApi
 */
class EdgefTwitterApi
{
    /**
    ...
    const REQUEST_TOKEN_FIELD = 'edgtf_twitter_request_token';
    const REQUEST_TOKEN_SECRET_FIELD = 'edgtf_twitter_request_token_secret';
    const ACCESS_TOKEN_FIELD = 'edgtf_twitter_access_token';
    const ACCESS_TOKEN_SECRET_FIELD = 'edgtf_twitter_access_token_secret';
    const AUTHORIZE_TOKEN_FIELD = 'edgtf_twitter_authorize_token';
    const AUTHORIZE_VERIFIER_FIELD = 'edgtf_twitter_authorize_verifier';
    const USER_ID_FIELD = 'edgtf_twitter_user_id';
    const USER_SCREEN_NAME_FIELD = 'edgtf_twitter_screen_name';

    /**
     * Private constructor because of singletone pattern. It sets all necessary properties
     */
    public function __construct() {
        $this->consumerKey = '2px4mFWY6MZQqrnK2fFboKqfG';
        $this->consumerSecret = 'FLtV71yLzHxd4tJJiWTET4ErU1m04esD4fHieQhsr5MkjDu1IP';
        $this->redirectURI = 'http://demo.edge-themes.com/twitter-app/twitter-redirect.php';
        ...
        $this->helper = new EdgefTwitterHelper();
    }
     ...
     *
     * @see EdgefTwitterApi::generateSignatureBase()
     */
    ...
    public function obtainRequestToken() {
        ...
        if (is_wp_error($response)) {
            ...
            $responseObj->message = esc_html__('Internal WP error', 'edgtf-twitter-feed');
        } else {
            ...
            if (!empty($responseBody)) {
                ...
                if ((is_array($responseParsed) && count($responseParsed)) &&
                    ...
                    if (!empty($responseObj->redirectUrl)) {
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Redirect URL couldn\'t be generated', 'edgtf-twitter-feed');
                    } else {
                        $responseObj->status = true;
                        $responseObj->message = esc_html__('Ok', 'edgtf-twitter-feed');
                    }
                } else {
                    $responseObj->status = false;
                    $responseObj->message = esc_html__('Couldn\'t connect with Twitter API', 'edgtf-twitter-feed');
                }
            }
        }
        ...
        if (!empty($authorizeVerifier) && !empty($authorizeToken)) {
            ...
            if (is_wp_error($response)) {
                ...
                if (is_array($responseParsed) && count($responseParsed)
                    ...
                    $responseObj->status = true;
                    $responseObj->message = esc_html__('Access token obtained', 'edgtf-twitter-feed');
                }
            }
        } else {
            $responseObj->status = false;
            $responseObj->message = esc_html__('Authorize token and it\'s secret were not obtained', 'edgtf-twitter-feed');
        }
    ...
    public function fetchTweets($userId = '', $count = '', $transient = array()) {
        ...
        if (!$this->transientExists($transient)) {
            if ($userId && $accessToken) {
                ...
                if (is_wp_error($response)) {
                    ...
                    if (isset($response['response']['code']) && $response['response']['code'] == 200) {
                        ...
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Couldn\'t connect with Twitter', 'edgtf-twitter-feed');
                    }
                }
            } else {
                $responseObj->status = false;
                $responseObj->message = esc_html__('It seems like you haven\'t connected with your Twitter account', 'edgtf-twitter-feed');
            }
        } else {
            ...
            if (!empty($transientContent)) {
                ...
                $responseObj->status = false;
                $responseObj->message = esc_html__('Couldn\'t retrieve content from database', 'edgtf-twitter-feed');
            }
        }
