...
class EdgefTwitterWidget extends WP_Widget
{
    ...
    public function __construct() {
        parent::__construct(
            ...
            esc_html__('Edge Twitter Widget', 'edgtf-twitter-feed'),
            array(
                'description' => esc_html__('Display your Twitter feed', 'edgtf-twitter-feed')
            )
        );
    ...
    private function setParams() {
        $this->params = array(
            array(
                'name' => 'title',
                'type' => 'textfield',
                'title' => esc_html__('Title', 'edgtf-twitter-feed')
            ),
            array(
                'name' => 'type_of_widget',
                'type' => 'dropdown',
                'title' => esc_html__('Type', 'edgtf-twitter-feed'),
                'options' => array(
                    'standard' => esc_html__('Standard', 'edgtf-twitter-feed'),
                    'slider' => esc_html__('Slider', 'edgtf-twitter-feed')
                )
            ),
            array(
                'name' => 'user_id',
                'type' => 'textfield',
                'title' => esc_html__('User ID', 'edgtf-twitter-feed')
            ),
            array(
                'name' => 'count',
                'type' => 'textfield',
                'title' => esc_html__('Number of Tweets', 'edgtf-twitter-feed')
            ),
            array(
                'name' => 'show_tweet_icon',
                'type' => 'dropdown',
                'title' => esc_html__('Show Tweet Icon', 'edgtf-twitter-feed'),
                'options' => array(
                    'yes' => esc_html__('Yes', 'edgtf-twitter-feed'),
                    'no' => esc_html__('No', 'edgtf-twitter-feed')
                )
            ),
            array(
                'name' => 'show_tweet_time',
                'type' => 'dropdown',
                'title' => esc_html__('Show Tweet Time', 'edgtf-twitter-feed'),
                'options' => array(
                    'no' => esc_html__('No', 'edgtf-twitter-feed'),
                    'yes' => esc_html__('Yes', 'edgtf-twitter-feed')
                )
            ...
            array(
                'name' => 'transient_time',
                'type' => 'textfield',
                'title' => esc_html__('Tweets Cache Time', 'edgtf-twitter-feed')
            )
        );
    }
        ...
        if ($twitter_api->hasUserConnected()) {
            ...
            if ($response->status) {
                if (is_array($response->data) && count($response->data)) { ?>
                    <ul class="edgtf-twitter-widget <?php echo esc_attr($holder_classes); ?>" <?php echo hyperon_edgtf_get_inline_attrs($slider_data); ?>>
                        <?php foreach ($response->data as $tweet) { ?>

                            <?php if ($type_of_widget == 'slider') { ?>

                                <li class="edgtf-tweet-holder">
                                    <div class="edgtf-tweet-text" <?php hyperon_edgtf_inline_style($twitter_style); ?>>
            ...
            esc_html_e('It seems that you haven\'t connected with your Twitter account', 'edgtf-twitter-feed');
        }
...
function edgtf_twitter_widget_load() {
    register_widget('EdgefTwitterWidget');
}

add_action('widgets_init', 'edgtf_twitter_widget_load');